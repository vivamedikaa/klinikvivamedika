      ...
      <div class="table-responsive">
        <table class="table table-bordered table-sm table-striped" id="pasien" width="100%" cellspacing="0">
          ...
          <tbody>
            ...
            <tr>
              <td>RM{{str_pad($pasien->id, 4, '0', STR_PAD_LEFT)  }}</td>
              <td>{{ $pasien->nama }}</td>
              <td>{{ $pasien->hp }}</td>
              <td width="120px">
      ...
      <form class="user" action="">
        ...
        <div class="form-group row">
          <div class="col-sm-6">
            ...
            <input type="text" class="form-control " name="asuransi" value="{{$iden->asuransi}}" readonly>
          </div>
          <div class="col-sm-6">
    ...
    <div class="card-body">
      <form class="user" action="{{route('rm.simpan')}}" method="post">
        ...
        <div class="form-group row">
          ...
          <input type="text" class="form-control " name="keluhan_utama" value="{{Request::old('keluhan_utama')}}" {{Auth::user()->profesi !== "Dokter" ? 'readonly': ''}} required>
        </div>
        <div class="form-group row">
          <label for="anamnesis">Anamnesis</label>
          <textarea type="date" class="form-control " name="anamnesis" required {{Auth::user()->profesi !== "Dokter" ? 'readonly': ''}}></textarea>
        </div>
        <div class="form-group row">
          <label for="pemeriksaan_fisik">Pemeriksaan Fisik</label>
          <textarea type="date" class="form-control " name="px_fisik" required {{Auth::user()->profesi !== "Dokter" ? 'readonly': ''}}></textarea>
        </div>
        <div class="form-group row">
          <div class="col-sm-6 mb-3 mb-sm-0">
...
<script type="text/javascript">
  ...
  function addresep() {
    ...
    if (resid !== null) {
      //code
      $("#reseps").append('<tr><td><input type="hidden" name="resep[' + a + '][id]" value="' + resid + '" class="form-control" readonly></td><td width="30%"><input type="text" name="resep[' + a + '][nama]" value="' + res + '" class="form-control" readonly></td><td width ="10%"><input type="text" name="resep[' + a + '][jumlah]" placeholder="Jumlah" class="form-control" required><td width="30%"><select name="resep[' + a + '][aturan]" class="form-control"><option value="3x1 Sesudah Makan">3x1 Sesudah Makan</option><option value="3x1 Sebelum Makan">3x1 Sebelum Makan</option><option value="2x1 Sesudah Makan">2x1 Sesudah Makan</option><option value="2x1 Sebelum Makan">3x1 Sebelum Makan</option><option value="1x1 Sesudah Makan">1x1 Sesudah Makan</option><option value="1x1 Sebelum Makan">1x1 Sebelum Makan</option></select></td><td><button type="button" class="btn btn-danger remove-res">Hapus</button></td></tr>');
    }
    ++a;
  };
